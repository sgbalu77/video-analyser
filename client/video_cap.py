...
def encode_and_send_frame(frame, frame_count, enable_kinesis=True, enable_rekog=True, write_file=True):
    try:
...
def main():
    ...
    while True:
        ...
        if frame_count % capture_rate == 0:
            result = pool.apply_async(encode_and_send_frame, (frame, frame_count, True, True, True,))
